	...
	\begin{frame}{Pensamento}
		\begin{columns}
			...
			\column{.6\textwidth}  		
				\begin{block}{Frase}
					\begin{center}
						{\large Nós somos aquilo que fazemos repetidamente. Excelência, então, não é um modo de agir, mas um hábito.}
					\end{center}
				\end{block}		  		
		  		\begin{block}{Quem?}
		  			\begin{center}
...
%------------------------------------------
	\section{Recuperação da Informação - RI}
	...
	\begin{frame}{RI {\it versus} Banco de Dados}
		...
		\begin{block}{Características...}
			Normalmente é permitido realizar consultas exatas (através de texto) \\ \pause
			\begin{center}
	...
	\begin{frame}{Dados não-estruturados em 1620}
		\begin{block}{Obras de Shakespeare}
			\begin{itemize}
				...
				e daí retirar as linhas que contêm 'Calpurnia'? \pause
				\item Por que não deveríamos fazer isto? \pause
				\begin{itemize}
					\item Lento (para coleções grandes); \pause
					\item NOT 'Calpurnia' não é trivial; \pause
	...
	\begin{frame}{Consultas por frases}
		\begin{block}{}
			\begin{itemize}
				...
				\item Então a sentença ``I went to university at Stanford? não é uma ocorrência; \pause
				\item O conceito de consultas por frases tem sido facilmente compreendido pelos usuários; \pause
				\item Uma das poucas ideias de ``busca avançada'' que funciona na prática; \pause
				\item Muitas das consultas são implicitamente consultas por frases; \pause
				\item Para isto, não é suficiente armazenar apenas entradas \\$\langle$ termo : docs $\rangle$.
			\end{itemize}
		\end{block}
	\end{frame}
